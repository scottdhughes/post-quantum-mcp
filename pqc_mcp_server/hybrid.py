...
# PGP Word List — 256 even-position words and 256 odd-position words.
# Alternating even/odd by byte position detects transposition errors.
# Source: https://en.wikipedia.org/wiki/PGP_word_list
_PGP_EVEN = (
    "aardvark",
    "absurd",
    "accrue",
...
def _fingerprint_words(public_key_bytes: bytes, word_count: int = 6) -> str:
    """Human-readable PGP Word List representation of a public key fingerprint.

    Uses alternating even/odd word dictionaries to detect transposition errors.
    Default 6 words (3 bytes) = 2^24 combinations (~16 million).
    """
    digest = hashlib.sha3_256(public_key_bytes).digest()
    byte_count = (word_count + 1) // 2  # 3 bytes for 6 words
    words = []
    for i in range(min(byte_count * 2, word_count)):
        byte_val = digest[i // 2] if i < byte_count * 2 else 0
        if i % 2 == 0:
            # Even position: use high nibble mapped to even words
            words.append(_PGP_EVEN[digest[i]])
        else:
            # Odd position: use odd words
            words.append(_PGP_ODD[digest[i]])
...
def _build_auth_transcript(
    ...
    Optional max_decrypt_time is appended last if present (sender-controlled TTL).
    """
    ...
    # Sender-controlled TTL: appended last, only if present (backwards compat)
    if max_decrypt_time:
        transcript += _len_prefix(max_decrypt_time)
    return transcript
...
def hybrid_keygen() -> dict[str, Any]:
    ...
    return {
        ...
        "classical": {
            ...
            "fingerprint_words": _fingerprint_words(x_pk_bytes),
        },
        "pqc": {
            ...
            "fingerprint_words": _fingerprint_words(pqc_pk),
        },
    }
...
def hybrid_auth_seal(
    ...
    max_decrypt_time: int | None = None,
) -> dict[str, Any]:
    ...
    max_decrypt_time: optional sender-controlled TTL in seconds. If set,
    the recipient cannot decrypt after timestamp + max_decrypt_time.
    """
    if max_decrypt_time is not None and max_decrypt_time < 0:
        raise ValueError("max_decrypt_time must be non-negative")
    ...
    mdt_bytes = str(max_decrypt_time).encode() if max_decrypt_time is not None else b""
    ...
    if max_decrypt_time is not None:
        result["max_decrypt_time"] = str(max_decrypt_time)
    return result
...
def _verify_authenticated_envelope(
    ...
    # Sender-controlled TTL (optional, signed in transcript if present)
    env_mdt = envelope.get("max_decrypt_time", "")
    mdt_bytes = str(env_mdt).encode() if env_mdt else b""
    transcript = _build_auth_transcript(
        version=envelope["version"].encode(),
        suite=envelope["suite"].encode(),
    ...
    # Sender-controlled TTL: refuse to decrypt expired envelopes.
    # This is sender-imposed (unlike max_age_seconds which is recipient-controlled).
    # The value is signed in the transcript, so it cannot be tampered with.
    if env_mdt and envelope_timestamp:
        try:
            mdt_int = int(env_mdt)
            ts_int_for_mdt = int(envelope_timestamp)
            if time.time() > ts_int_for_mdt + mdt_int:
                raise ValueError(
                    f"Envelope has expired (sender-imposed TTL of {mdt_int}s exceeded). "
                    "The sender set this message to self-destruct."
                )
        except (TypeError, OverflowError) as exc:
            raise ValueError(f"Invalid max_decrypt_time: {exc}") from exc
